...
import (
	...
	"github.com/astralink/astralink-go/internal/client"
...
func printClientUsage(fs *flag.FlagSet) {
	...
	if bin == "" || bin == "." || strings.Contains(bin, "go-build") || strings.HasPrefix(bin, "main") {
		bin = "astralink-client"
	}

	fmt.Fprintf(fs.Output(), "AstraLink Client - A high-performance DNS-based VPN Tunnel\n\n")
...
func newClientFlagSet(output io.Writer) (*flag.FlagSet, *clientCLIOptions, *config.ClientConfigFlagBinder, error) {
	fs := flag.NewFlagSet("astralink-client", flag.ContinueOnError)
	fs.SetOutput(output)
...
func main() {
	...
	if opts.showVersion {
		fmt.Printf("AstraLink Client Version: %s\n", version.GetVersion())
		return
	}
	...
	log := app.Log()
	if log != nil {
		log.Infof("\U0001F680 <green>AstraLink Client Started</green>")
