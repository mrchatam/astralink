...
import (
	...
	"github.com/astralink/astralink-go/internal/runtimepath"
	"github.com/astralink/astralink-go/internal/security"
	UDPServer "github.com/astralink/astralink-go/internal/udpserver"
	"github.com/astralink/astralink-go/internal/version"
)
...
func main() {
	...
	if *versionFlag {
		fmt.Printf("AstraLink Server Version: %s\n", version.GetVersion())
		return
	}
	...
	if *logPath != "" {
		log = logger.NewWithFile("AstraLink Server", cfg.LogLevel, *logPath)
	} else {
		log = logger.New("AstraLink Server", cfg.LogLevel)
	}

	log.Infof("============================================================")
	log.Infof("<cyan>GitHub:</cyan> <yellow>https://github.com/astralink</yellow>")
	log.Infof("<cyan>Telegram:</cyan> <yellow>AstraLink</yellow>")
	log.Infof("<cyan>Build Version:</cyan> <yellow>%s</yellow>", version.GetVersion())
	log.Infof("============================================================")

	log.Infof("\U0001F680 <magenta>AstraLink Server starting ...</magenta>")
	...
	authMode := authority.ModeSingle
	switch strings.ToLower(strings.TrimSpace(cfg.AuthorityMode)) {
	case "multi":
		authMode = authority.ModeMulti
	case "anycast":
		authMode = authority.ModeAnycast
	}
	if cfg.AnycastEnabled {
		authMode = authority.ModeAnycast
	}
	authRegistry := authority.NewRegistry(authMode, cfg.AuthorityPeers)
	if node, ok := authRegistry.Select(); ok {
		log.Infof("Authority registry mode=<cyan>%s</cyan> selected=<yellow>%s</yellow>", authRegistry.Mode(), node.Address)
	}
