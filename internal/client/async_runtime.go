...
import (
	...
	"github.com/astralink/astralink-go/internal/arq"
	"github.com/astralink/astralink-go/internal/client/handlers"
	DnsParser "github.com/astralink/astralink-go/internal/dnsparser"
	Enums "github.com/astralink/astralink-go/internal/enums"
	fragmentStore "github.com/astralink/astralink-go/internal/fragmentstore"
)
...
func (c *Client) buildPlannedOutboundFrames(
	...
	rawEncoded, err := c.buildEncodedAutoWithCompressionTrace(task.opts)
	if err != nil {
		return nil, err
	}
	...
	for _, resolverConn := range conns {
		...
		case firstDNSPacket == nil:
			...
			if c.transportShouldBundle(task) {
				ednsSize = c.transportEDNSSizeForPath(resolverConn.Key)
			}
			pathEncoded := c.transportWrapPayloadForSend(task, rawEncoded, resolverConn.Key)
			dnsPacket, err = DnsParser.BuildTunnelTXTQuestionPacketPrepared(prepared.normalized, prepared.qname, pathEncoded, Enums.DNS_RECORD_TYPE_TXT, uint16(ednsSize))
			if err != nil {
				continue
			}
			firstDomain = domain
		...
		frames = append(frames, encodedOutboundDatagram{
			...
			packet:      dnsPacket,
			sendBytes:   len(rawEncoded) + 128,
			streamID:    task.opts.StreamID,
			sequenceNum: task.opts.SequenceNum,
			packetType:  task.opts.PacketType,
		})
	}

	if parity := c.transportMaybeFECParity(task, rawEncoded); len(parity) > 0 && len(conns) > 0 {
		parityOpts := task.opts
		parityOpts.Payload = parity
		if parityEncoded, err := c.buildEncodedAutoWithCompressionTrace(parityOpts); err == nil {
			conn := conns[0]
			domain := conn.Domain
			if domain == "" {
				domain = defaultDomain
			}
			if prep, err := prepareTunnelDomain(domain); err == nil {
				ednsSize := c.transportEDNSSizeForPath(conn.Key)
				wrapped := c.transportWrapPayloadForSend(task, parityEncoded, conn.Key)
				if pkt, err := DnsParser.BuildTunnelTXTQuestionPacketPrepared(prep.normalized, prep.qname, wrapped, Enums.DNS_RECORD_TYPE_TXT, uint16(ednsSize)); err == nil {
					addr, _ := c.getResolverUDPAddr(conn)
					frames = append(frames, encodedOutboundDatagram{
						addr:        addr,
						serverKey:   conn.Key,
						packet:      pkt,
						sendBytes:   len(parityEncoded) + 128,
						streamID:    task.opts.StreamID,
						sequenceNum: task.opts.SequenceNum,
						packetType:  task.opts.PacketType,
					})
		...
		case task, ok := <-c.encodedTXChannel:
			...
			for _, frame := range task.frames {
				...
				if _, err := conn.WriteToUDP(frame.packet, frame.addr); err == nil {
					...
					sendBytes := frame.sendBytes
					if sendBytes <= 0 {
						sendBytes = len(frame.packet)
					}
					c.transportReportSent(frame.serverKey, frame.packet, sendBytes, frame.streamID, frame.sequenceNum, frame.packetType)
				} else if frame.serverKey != "" {
					sendBytes := frame.sendBytes
					if sendBytes <= 0 {
						sendBytes = len(frame.packet)
					}
					c.transportReleaseReserved([]string{frame.serverKey}, sendBytes)
				}
			}
			if !task.wasPacked && task.selected != nil {
				task.selected.ReleaseTXPacket(task.item)
...
func (c *Client) handleInboundPacket(data []byte, addr *net.UDPAddr, localAddr string) {
	...
	if err != nil {
		if errors.Is(err, DnsParser.ErrTXTAnswerMissing) {
			...
			if parsed, parseErr := DnsParser.ParsePacketLite(data); parseErr == nil && parsed.Header.RCode != 0 {
				if sample, ok := c.balancer.PeekPendingSample(data, addr, localAddr); ok {
					c.transportReportLost(sample.ServerKey, sample.DNSID, 0)
				}
				...
				if sample, ok := c.balancer.PeekPendingSample(data, addr, localAddr); ok {
					...
					c.balancer.TrackResolverSuccess(data, addr, localAddr, receivedAt, rtt)
					c.transportReportDNSDelivered(sample.ServerKey, sample.DNSID, rtt)
				} else {
					c.balancer.TrackResolverSuccess(data, addr, localAddr, receivedAt, 0)
				}
			}
	...
	if sample, ok := c.balancer.PeekPendingSample(data, addr, localAddr); ok {
		...
		if vpnPacket.PacketType == Enums.PACKET_STREAM_DATA_ACK {
			// Stream-level confirmation handled in preprocessInboundPacket.
		} else {
			c.transportReportDNSDelivered(sample.ServerKey, sample.DNSID, rtt)
		}
	} else {
