...
func (c *Client) nextSessionInitAttempt() (Connection, []byte, [4]byte, error) {
	...
	active = c.preferAuthorityConnections(active)
	if len(c.cfg.AuthorityEndpoints) > 1 {
		if fb := c.authorityFallbackConnections(Connection{}); len(fb) > 0 {
			active = c.preferAuthorityConnections(fb)
		}
	}
