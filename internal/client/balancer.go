...
type Balancer struct {
	...
	onResolverTimeout        func(serverKey string, dnsID uint16)
}
...
func (b *Balancer) SetResolverTimeoutHandler(handler func(serverKey string, dnsID uint16)) {
	if b == nil {
		return
	}
	b.mu.Lock()
	b.onResolverTimeout = handler
	b.mu.Unlock()
}
...
// PendingSampleInfo describes an in-flight resolver exchange.
type PendingSampleInfo struct {
	ServerKey string
	SentAt    time.Time
	DNSID     uint16
}

func (b *Balancer) PeekPendingServerKey(packet []byte, addr *net.UDPAddr, localAddr string) string {
	info, ok := b.PeekPendingSample(packet, addr, localAddr)
	if !ok {
		return ""
	}
...
func (b *Balancer) PeekPendingSample(packet []byte, addr *net.UDPAddr, localAddr string) (PendingSampleInfo, bool) {
	var empty PendingSampleInfo
...
// PathTelemetry returns RTT and loss estimates for transport feedback sync.
func (b *Balancer) PathTelemetry(serverKey string) (time.Duration, float64, float64) {
	stats := b.statsForKey(serverKey)
	if stats == nil {
		return 0, 0, 0
	}
	sent, acked, lost, rttSum, rttCount := stats.snapshot()
	var loss, timeout float64
	if sent > 0 {
		loss = float64(lost) / float64(sent)
		timeout = float64(lost) / float64(sent)
	}
	var rtt time.Duration
	if rttCount > 0 {
		rtt = time.Duration(rttSum/rttCount) * time.Microsecond
	}
	_ = acked
	return rtt, loss, timeout
}
...
func (b *Balancer) CollectExpiredResolverTimeouts(
	...
	b.mu.RLock()
	timeoutHandler := b.onResolverTimeout
	b.mu.RUnlock()
	needTransportSweep := timeoutHandler != nil
	if !autoDisable && !needTransportSweep {
		return
	}
	...
	for _, observation := range timeoutObservations {
		if timeoutHandler != nil && observation.serverKey != "" {
			timeoutHandler(observation.serverKey, observation.dnsID)
		}
		if autoDisable {
