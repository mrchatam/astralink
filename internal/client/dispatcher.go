...
func (c *Client) asyncStreamDispatcher(ctx context.Context) {
	...
	for {
		...
		if c.transport != nil && !c.transportShouldBundle(probeTask) {
			maxBlocks = 1
		}
		...
		if VpnProto.IsPackableControlPacket(item.PacketType, len(item.Payload)) && maxBlocks > 1 {
			...
			if blocks > 1 {
				...
				wasPacked = true
				c.transportRecordBundleExecution(blocks, true)
				if selected != nil {
					selected.ReleaseTXPacket(item)
				}
				item = nil
		...
		task.dupCount = c.transportDupCount(task)
