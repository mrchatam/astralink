...
import (
	...
	dnsCache "github.com/astralink/astralink-go/internal/dnscache"
	...
	"github.com/astralink/astralink-go/internal/mlq"
...
type Client struct {
	...
	// Multipath QUIC-over-DNS transport core
	transport          *transport.Runtime
	authorityBootstrap authority.BootstrapInfo
	fecInboundCache    *fecInboundState
}
...
type plannerTask struct {
	...
	transportPlan  transport.PlanResult
	hasTransportPlan bool
}
...
func BootstrapLoadedConfig(cfg config.ClientConfig, logPath string) (*Client, error) {
	...
	if logPath != "" {
		log = logger.NewWithFile("AstraLink Client", cfg.LogLevel, logPath)
	} else {
		log = logger.New("AstraLink Client", cfg.LogLevel)
	}
...
func (c *Client) HandleStreamPacket(packet VpnProto.Packet) error {
	...
	case Enums.PACKET_STREAM_DATA, Enums.PACKET_STREAM_RESEND:
		var fecHandled bool
		packet, fecHandled = c.transportHandleFECInbound(packet)
		if fecHandled {
			return nil
		}
		if arqObj.IsClosed() {
