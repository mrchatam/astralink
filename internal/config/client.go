...
import (
	...
	"github.com/astralink/astralink-go/internal/compression"
)
...
func finalizeClientConfig(cfg ClientConfig) (ClientConfig, error) {
	...
	if strings.TrimSpace(cfg.Transport) == "" {
		cfg.Transport = "multipath_quic_dns"
	}
	if strings.TrimSpace(cfg.TransportMode) == "" {
		cfg.TransportMode = "simple"
	}
	cfg.TransportMode = strings.ToLower(strings.TrimSpace(cfg.TransportMode))
	if cfg.TransportMode != "simple" && cfg.TransportMode != "advanced" {
		return cfg, fmt.Errorf("invalid MODE: %q (use simple or advanced)", cfg.TransportMode)
	}
	if cfg.MaxActivePaths <= 0 {
		if cfg.TransportMode == "advanced" {
			cfg.MaxActivePaths = 4
		} else {
			cfg.MaxActivePaths = 1
		}
	}
	if cfg.MaxStandbyPaths <= 0 {
		if cfg.TransportMode == "advanced" {
			...
			cfg.MaxStandbyPaths = 1
		}
	}
	if cfg.CongestionProfile == "" {
		cfg.CongestionProfile = "conservative"
	}
	if cfg.MaxBundleBytes <= 0 {
		cfg.MaxBundleBytes = 512
	}
	if cfg.TransportMode == "simple" {
		cfg.FECEnabled = false
		cfg.PacketDuplicationCount = clampInt(defaultIntBelow(cfg.PacketDuplicationCount, 1, 1), 1, 3)
	} else {
		cfg.FECEnabled = cfg.FECEnabled || cfg.FECParityShards > 0
	}
