...
import (
	...
	domainMatcher "github.com/astralink/astralink-go/internal/domainmatcher"
...
func (s *Server) handleTunnelCandidate(packet []byte, parsed DnsParser.LitePacket, decision domainMatcher.Decision) []byte {
	var (
		vpnPacket VpnProto.Packet
		err       error
	)
	if s.cfg.TransportAware {
		vpnPacket, _, err = parseInflatedFromTunnelLabels(decision.Labels, s.codec)
	} else {
